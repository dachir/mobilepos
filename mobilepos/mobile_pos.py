...
def create_guest_order():
    try:
        request_data = frappe.request.data
        request_dict = frappe.parse_json(request_data.decode("utf-8")) or {}
        data = request_dict.get("data") if isinstance(request_dict.get("data"), dict) else request_dict

        frappe.log_error("Guest Order Creation", f"Received guest order creation request: {data}")

        guest_info = {
            "custom_is_guest_order": 1,
            "custom_address_email": data.get("email"),
            "custom_first_name": data.get("first_name"),
            "custom_last_name": data.get("last_name"),
            "custom_address_line_01": data.get("address_line1"),
            "custom_address_line_02": data.get("address_line2"),
            "custom_address_line_in_arabic": data.get("address_in_arabic"),
            "custom_address_city": data.get("address_city"),
            "custom_address_county": data.get("address_county"),
            "custom_address_state": data.get("address_state"),
            "custom_address_country": data.get("address_country"),
            "custom_address_pin_code": data.get("address_pincode"),
            "custom_address_phone": data.get("address_phone"),
            "custom_address_fax": data.get("address_fax"),
            "custom_longitude": data.get("coustome_longitude") or 0,
        ...
        cart = data.get("cart")
        if not cart:
            frappe.throw("Guest order creation failed: Cart data is missing")

        # ✅ Idempotence: éviter de créer 10 SO si l’app retente
        threshold = add_to_date(now_datetime(), hours=-24)
        existing_so = frappe.get_all(
            "Sales Order",
            filters={
                "custom_is_guest_order": 1,
                "custom_address_email": email,
                "creation": (">=", threshold),
                "docstatus": 0,
            },
        ...
        if existing_so:
            order_name = existing_so[0]["name"]

            # si log incomplet ou absent => on relance l’async (idempotent)
            frappe.enqueue(
            ...
            return {"success": True, "order_name": order_name, "mode": "idempotent_return"}

        # ✅ on ne bloque plus si Completed existe (redux peut être incohérent)
        # on crée le SO guest puis le worker décidera s’il faut (re)créer/compléter.

        response = create_order(**data)
        order_name = response.get("name") if isinstance(response, dict) else getattr(response, "name", None)
        if not order_name:
            frappe.throw("Order creation failed (no order name).")

        # Injection des champs guest
        for fieldname, value in guest_info.items():
            if value is not None and value != "":
                frappe.db.set_value("Sales Order", order_name, fieldname, value)

        frappe.log_error("Guest Order Creation", f"Enqueuing account creation for order {order_name} / email {email}")
        frappe.enqueue(
            method=create_user_and_customer,
            queue="default",
            timeout=300,
        ...
        return {"success": True, "order_name": order_name, "mode": "created"}
...
def create_user_and_customer(guest_data=None, order_name=None):
    ...
    if not guest_data:
        request_data = frappe.request.data
        request_dict = frappe.parse_json(request_data.decode('utf-8')) or {}
        guest_data = request_dict.get("data", {}) if isinstance(request_dict.get("data"), dict) else request_dict
    ...
    try:
        ...
        # -------------------------
        if is_background:
            email = (guest_data.get("custom_address_email") or "").strip().lower()
            first_name = (guest_data.get("custom_first_name") or "").strip()
            last_name = (guest_data.get("custom_last_name") or "").strip()
            mobile_no = (guest_data.get("custom_address_phone") or "").strip()
            longitude = guest_data.get("custom_longitude") or 0
            latitude = guest_data.get("custom_latitude") or 0

            address_data = {
                "address_title": f"{first_name} {last_name}".strip() or email,
            ...
            mobile_no = (guest_data.get("mobile_no") or "").strip()
            address_data = guest_data.get("address_data", {}) or {}

        # -------------------------
        # 2) Fallback si email vide (création via téléphone)
        #    => user/prenom = phone (digits only)
        # -------------------------
        phone_digits = only_digits(mobile_no) if mobile_no else ""
        if not email:
            if not phone_digits:
                return {"error": "Missing required fields (email or mobile)."}
            email = f"{phone_digits}@mobile.com"
            first_name = phone_digits
            last_name = phone_digits

        if not first_name:
            # si vide, fallback propre
            first_name = (phone_digits or email)
        ...
        if not last_log:
            rows = frappe.get_all(
                ...
                filters={"email": email},
                fields=["name", "status", "customer_code", "public_key", "private_key", "creation", "retry_count"],
                order_by="creation desc",
                limit_page_length=1
            )
            last_log = rows[0] if rows else None

        def log_is_older_than_8h(log_row):
            if not log_row or not log_row.get("creation"):
                return True
            return log_row["creation"] < add_to_date(now_datetime(), hours=-8)

        # -------------------------
        # 4) Si log incomplet et vieux => on "relance"
        #    (nouveau log Pending, retry_count++)
        # -------------------------
        log_doc = None
        if is_background:
            need_new_log = False
            if not last_log:
                need_new_log = True
            elif last_log.get("status") != "Completed" and log_is_older_than_8h(last_log):
                need_new_log = True
            ...
            if need_new_log:
                log_doc = frappe.get_doc({
                    ...
                    "status": "Pending",
                    "retry_count": (last_log.get("retry_count") or 0) + 1 if last_log else 0
                })
                log_doc.insert(ignore_permissions=True)
            else:
                # utiliser le dernier log si possible
                if last_log and last_log.get("name"):
                    log_doc = frappe.get_doc("App Registration Log", last_log["name"])

        # -------------------------
        # 5) Ensure User (idempotent)
        # -------------------------
        ...
        # Keys: si on est en réparation / relance, on régénère le secret
        old_user = frappe.session.user
        try:
            frappe.set_user("Administrator")
            keys = generate_keys(user_doc.name)
        finally:
            frappe.set_user(old_user)

        private_key = keys.get("api_secret")
        public_key = frappe.db.get_value("User", user_doc.name, "api_key")

        # -------------------------
        # 6) Ensure Customer (idempotent)
        # -------------------------
        ...
        if not existing_customer:
            ...
            customer_doc = frappe.get_doc({
                ...
                "name": new_customer_code,
                "custom_customer_code": new_customer_code,
                "customer_name": f"{first_name} {last_name}".strip() or first_name,
                "email_id": email,
                "mobile_no": mobile_no if frappe.get_meta("Customer").has_field("mobile_no") else None,
                "customer_group": "App Customer Group",
                "territory": "All Territories",
                "customer_type": "Individual"
            })
        ...
        # -------------------------
        # 7) Ensure Address (idempotent)
        #    si latitude/longitude absents => 0
        # -------------------------
        if address_data:
            address_data["custom_longitude"] = address_data.get("custom_longitude") or 0
            address_data["custom_latitude"] = address_data.get("custom_latitude") or 0

            addr_name_guess = f"{new_customer_code}_01"
            if not frappe.db.exists("Address", addr_name_guess):
                create_address_2(address_data, new_customer_code)

        # -------------------------
        # 8) Patch Sales Order (si fourni)
        # -------------------------
        ...
        # 9) Finaliser log
        # -------------------------
        if is_background and log_doc:
            log_doc.status = "Completed"
            log_doc.customer_code = new_customer_code
            log_doc.public_key = public_key
            log_doc.private_key = private_key
        ...
        try:
            if is_background:
                # essayer de marquer Failed le dernier log du run
                if 'log_doc' in locals() and log_doc:
                    log_doc.status = "Failed"
                    log_doc.error_log = str(e)
                    log_doc.retry_count = (log_doc.retry_count or 0) + 1
                    log_doc.save(ignore_permissions=True)
        except Exception:
            pass
...
def check_user_registration_status(email=None, mobile=None):
    ...
    mobile = (mobile or frappe.form_dict.get("mobile") or "").strip()
    ...
    try:
        log = frappe.get_all(
            "App Registration Log",
            filters=filters,
            fields=["status", "customer_code", "email", "public_key", "private_key", "sales_order", "creation"],
            order_by="creation desc",
            limit_page_length=1
        )

        if not log:
            return {"status": "Not Found", "registered": False, "can_guest": True}
        ...
        return {
            ...
            "can_guest": status != "Completed",  # ✅ si incomplet => guest mode
            "customer_code": entry["customer_code"] if status == "Completed" else None,
            "public_key": entry.get("public_key"),
            "private_key": entry.get("private_key"),
            "user_email": entry.get("email"),
            "order": entry.get("sales_order"),
        }
